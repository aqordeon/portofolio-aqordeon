                                    ...
                                    <div>
                                        <div class="flex items-center">
                                            <StarIcon v-for="rating in [0, 1, 2, 3, 4]" :key="rating"
                                                :class="[reviews.average > rating ? 'text-yellow-400' : 'text-gray-300', 'size-5 shrink-0']"
                                                aria-hidden="true" />
                                        </div>
                            ...
                            <div class="sm:flex sm:justify-between">
                                ...
                                <fieldset>
                                    ...
                                    <div class="mt-1 grid grid-cols-1 gap-4 sm:grid-cols-2">
                                        <label v-for="size in product.sizes" :key="size.name" :aria-label="size.name"
                                            ...
                                            <input type="radio" name="size" :value="size.name"
                                                :checked="size === product.sizes[0]"
                                                class="absolute inset-0 appearance-none focus:outline focus:outline-0" />
                                            <div class="flex-1">
                                                <span class="block text-base font-medium text-gray-900">{{ size.name
                                                }}</span>
                                                <span class="mt-1 block text-sm text-gray-500">{{ size.description
                                                }}</span>
                                            </div>
                                            <CheckCircleIcon
                            ...
                            <div class="mt-4">
                                <a href="#" class="group inline-flex text-sm text-gray-500 hover:text-gray-700">
                                    ...
                                    <QuestionMarkCircleIcon
                                        class="ml-2 size-5 shrink-0 text-gray-400 group-hover:text-gray-500"
                                        aria-hidden="true" />
                                </a>
                            </div>
                            <div class="mt-10">
